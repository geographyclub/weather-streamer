...
for (( counter = 1; counter <= ${count}; counter++ )); do
  convert -size ${width}x${height} xc:black $PWD/../data/tmp/layer0_$(echo ${extent[*]} | tr ' ' '_').tif -gravity center -compose over -composite $PWD/../data/tmp/layer1_$(echo ${extent[*]} | tr ' ' '_')_$(printf "%06d" ${counter}).tif -gravity center -compose over -composite -level 50%,100% $PWD/../data/tmp/frame_$(printf "%06d" ${counter}).tif
done

### stream
ls -tr $PWD/../data/tmp/frame_*.tif | sed -e "s/^/file '/g" -e "s/$/'/g" > $PWD/../data/tmp/filelist.txt
ffmpeg -y -r 12 -f concat -safe 0 -i $PWD/../data/tmp/filelist.txt -c:v libx264 -crf 23 -pix_fmt yuv420p -preset fast -threads 0 -movflags +faststart $PWD/../extent_clouds_$(echo ${extent[*]} | tr ' ' '_')_$(date +%m_%d_%H%M%S).mp4
