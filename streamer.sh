...
#gdalwarp -ts ${height} ${width} -r cubicspline

### make color gradients
#convert -size 10x1024 gradient:navy-snow $PWD/../data/colors/ice-sea.png

### slideshow from images
convert -delay 50 $PWD/../data/qgis/places/*.png $PWD/../places.gif

### gdps animation
# unprojected
field=TCDC
color=$PWD/../data/colors/white-black.txt
counter=1
rm -f $PWD/../data/tmp/*.tif
ls $PWD/../data/gdps/*${field}*.grib2 | while read file; do
  gdaldem color-relief -f 'GRIB' -of 'GTiff' --config GDAL_PAM_ENABLED NO ${file} ${color} /vsistdout/ | gdalwarp -overwrite -f 'GTiff' -of 'GTiff' --config GDAL_PAM_ENABLED NO -co PROFILE=BASELINE /vsistdin/ $PWD/../data/tmp/$(printf "%06d" ${counter}).tif
  (( counter = counter + 1 ))
done
ffmpeg -y -r 10 -i $PWD/../data/tmp/%06d.tif -s ${width}x${height} -c:v libx264 -crf 23 -pix_fmt yuv420p -preset fast -threads 0 -movflags +faststart $PWD/../${field}_unprojected.mp4
...
rm -f $PWD/../data/tmp/*.tif
ls $PWD/../data/gdps/*${field}*.grib2 | while read file; do
  gdaldem color-relief -f 'GRIB' -of 'GTiff' --config GDAL_PAM_ENABLED NO ${file} ${color} /vsistdout/ | gdalwarp -overwrite -f 'GTiff' -of 'GTiff' --config GDAL_PAM_ENABLED NO -co PROFILE=BASELINE -t_srs '+proj=ortho +lat_0=-10 +lon_0=-60' /vsistdin/ $PWD/../data/tmp/$(printf "%06d" ${counter}).tif
  (( counter = counter + 1 ))
done
ffmpeg -y -r 10 -i $PWD/../data/tmp/%06d.tif -vf "scale=${width}:-2" -c:v libx264 -crf 23 -pix_fmt yuv420p -preset fast -threads 0 -movflags +faststart $PWD/../${field}_ortho_west.mp4
