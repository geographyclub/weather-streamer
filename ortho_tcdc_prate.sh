...
counter=1
ls ${dir}/maps/gdps/*PRATE*.grib2 | while read file; do
  gdaldem color-relief -alpha -f 'GRIB' -of 'GTiff' --config GDAL_PAM_ENABLED NO ${file} "${dir}/maps/colors/precip.txt" /vsistdout/ | gdalwarp -overwrite -dstalpha --config GDAL_PAM_ENABLED NO -co PROFILE=BASELINE -f 'GTiff' -of 'GTiff' -s_srs 'EPSG:4326' -t_srs '+proj=ortho +lat_0='${lat_0}' +lon_0='${lon_0}' +ellps='sphere'' -ts 0 ${height} -r cubicspline /vsistdin/ ${dir}/tmp/layer2_${lon_0}_${lat_0}_$(printf "%06d" ${counter}).tif
  (( counter = counter + 1 )) 
done
...
for (( counter = 1; counter <= ${count}; counter++ )); do
  convert -size ${width}x${height} xc:black \( ${dir}/tmp/layer0_${lon_0}_${lat_0}.tif -resize 75% \) -gravity center -compose over -composite \( ${dir}/tmp/layer1_${lon_0}_${lat_0}_$(printf "%06d" ${counter}).tif -resize 75% \) -gravity center -compose over -composite \( ${dir}/tmp/layer2_${lon_0}_${lat_0}_$(printf "%06d" ${counter}).tif -resize 75% \) -gravity center -compose multiply -composite -level 50%,100% ${dir}/tmp/frame_$(printf "%06d" ${counter}).tif
done
